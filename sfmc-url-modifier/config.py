...
# Le code dans le nom de la journey détermine le remplacement d'URL
COUNTRY_URL_MAPPINGS = {
    ...
    'EN': ('en', 'en-gb'),
    'AT': ('at', 'de-at'),
    'FF': ('fr', 'fr-fr'),      # FF = France (variante)
    'FT': ('fr', 'fr-fr'),      # FT = France Test
    'SITE': ('fr', 'fr-fr'),    # SITE = France par défaut
}
...
def extract_country_from_name(journey_name):
    ...
    Patterns supportés:
    - JB-S-ET-FR-xxx ou JB-O-ET-FR-xxx (transactional)
    - S-FR-Welcome ou O-CJ-xxx-FR-Welcome (welcome)
    - S-FR-xxx-V2 (welcome v2)
    """
    ...
    # Pattern welcome: S-{COUNTRY}-Welcome ou O-CJ-xxx-{COUNTRY}-Welcome
...
def get_url_patterns_for_journey(journey_name):
    ...
    Retourne None si le pays n'est pas détecté ou non supporté.
    """
